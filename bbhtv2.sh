...
sudo add-apt-repository -y ppa:apt/stable < /dev/null
sudo echo debconf apt/maxdownloads string 16 | debconf-set-selections
sudo echo debconf apt/dlflag boolean true | debconf-set-selections
sudo echo debconf apt/aptmanager string apt-get | debconf-set-selections
...
sudo apt install -y nmap phantomjs 
...
sudo go install github.com/OJ/gobuster@latest
echo "${BLUE} done${RESET}"
echo ""
...
sudo pip3 install awscli -pgrade -ser
...
go install  github.com/projectdiscovery/naabu/cmd/naabu
echo "${BLUE} done${RESET}"
echo ""
